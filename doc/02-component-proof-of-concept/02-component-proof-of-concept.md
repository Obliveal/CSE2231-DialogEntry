...
I decided to implement a Dialog Tree component. I picked it because I think it has a small enough scope that it will not be too difficult to implement, but challenging enough that it's not trivial. I changed the methods a little as well:
- Kernel Methods:
  ...
  - `Sequence<DialogEntry> getResponses()`: gets each of the child dialog entries as a sequence
- Secondary Methods:
  - `boolean hasResponse()`: checks if the dialog entry has a response
  - `Set<String> getSpeakers()`: gets all of the speakers in all of the dialog entry subtrees
